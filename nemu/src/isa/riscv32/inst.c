...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli,   R, R(rd) = src1 << SHAMT(s->isa.inst));

  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul,    R, R(rd) = SIGN(src1) * SIGN(src2); printf("rd:%x  rs1:%x  rs2:%x\n", R(rd), src1, src2));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh,   R, R(rd) = BITS((SIGNH(src1) * SIGNH(src2)), 63, 32); printf("mul:%x  rd:%x  rs1:%x  rs2:%x\n", (uint32_t)(((int64_t)src1 * (int64_t)src2) >> 32), R(rd), src1, src2));
